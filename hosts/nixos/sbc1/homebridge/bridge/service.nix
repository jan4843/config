  ...
  self.freeform.homebridge = {
    ...
    compose.services.homebridge = {
      ...
      build.dockerfile_inline = ''
        ...
        RUN \
          apt-get update && \
          apt-get install -y ${args.lib.escapeShellArgs (args.lib.naturalSort args.config.self.freeform.homebridge.packages.apt)} && \
          rm -rf /var/lib/apt/lists/*
        RUN \
          npm install --global ${args.lib.escapeShellArgs (args.lib.naturalSort args.config.self.freeform.homebridge.packages.npm)}
      '';
